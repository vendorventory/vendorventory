...
import { 
  LayoutDashboard, FileText, ShoppingBag, Wallet, 
  Gavel, UserCheck, Settings, HelpCircle, 
  Home, MoreHorizontal, X, User
} from 'lucide-react';

const buyerMenuItems = [
  { icon: LayoutDashboard, label: 'Dashboard', href: '/buyer/dashboard' },
  { icon: FileText, label: 'My Invoices', href: '/buyer/invoices' },
  { icon: ShoppingBag, label: 'Orders & Tracking', href: '/buyer/orders' },
  { icon: Wallet, label: 'Payments / Escrow', href: '/buyer/escrow' },
  { icon: Gavel, label: 'Disputes', href: '/buyer/disputes' },
  { icon: UserCheck, label: 'Profile & KYC', href: '/buyer/profile' },
  { icon: Settings, label: 'Settings', href: '/buyer/settings' },
];

const mobileNavItems = [
  { icon: Home, label: 'Home', href: '/buyer/dashboard' },
  { icon: FileText, label: 'Invoices', href: '/buyer/invoices' },
  { icon: ShoppingBag, label: 'Orders', href: '/buyer/orders' },
  { icon: Wallet, label: 'Escrow', href: '/buyer/escrow' },
];
  ...
  return (
    <>
          ...
          <div className="bg-white/5 rounded-xl p-3 border border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[13px] font-bold text-white truncate">Chioma Nwosu</p>
                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                  <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-tight">Verified Buyer</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="lg:hidden fixed bottom-6 left-0 right-0 px-4 z-[100]">
        <nav className="max-w-md mx-auto bg-white/90 backdrop-blur-md border border-white/20 flex justify-around items-center py-3 px-2 rounded-[24px] shadow-[0_10px_30_rgba(0,0,0,0.15)] ring-1 ring-black/5">
