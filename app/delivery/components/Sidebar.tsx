...
import { 
  LayoutDashboard, Truck, Users, Wallet, 
  Gavel, MapPin, UserCheck, Settings, 
  HelpCircle, Home, MoreHorizontal, X, User
} from 'lucide-react';

// Menu items specific to Delivery Partners
const deliveryMenuItems = [
  { icon: LayoutDashboard, label: 'Dashboard', href: '/delivery/dashboard' },
  { icon: Truck, label: 'Deliveries', href: '/delivery/deliveries' },
  { icon: Users, label: 'Riders / Drivers', href: '/delivery/riders' },
  { icon: Wallet, label: 'Earnings & Payouts', href: '/delivery/earnings' },
  { icon: Gavel, label: 'Disputes', href: '/delivery/disputes' },
  { icon: MapPin, label: 'Service Zones', href: '/delivery/zones' },
  { icon: UserCheck, label: 'Profile & Verification', href: '/delivery/profile' },
  { icon: Settings, label: 'Settings', href: '/delivery/settings' },
];

const mobileNavItems = [
  { icon: Home, label: 'Home', href: '/delivery/dashboard' },
  { icon: Truck, label: 'Deliveries', href: '/delivery/deliveries' },
  { icon: Users, label: 'Riders', href: '/delivery/riders' },
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex fixed top-0 left-0 z-[55] w-[280px] h-screen bg-[#19246a] flex-col">
        ...
        {/* Delivery Partner Profile Summary */}
          ...
          <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[13px] font-black text-white truncate">SwiftLogistics Nigeria</p>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Logistics Partner</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:hidden fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm p-4 flex flex-col justify-end">
          <div className="bg-white rounded-[32px] p-6 pb-8 shadow-2xl animate-in slide-in-from-bottom-10 duration-300">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-black text-[#19246a] text-lg uppercase tracking-widest">Partner Menu</h3>
      ...
      <div className="lg:hidden fixed bottom-6 left-0 right-0 px-4 z-[100]">
        <nav className="max-w-md mx-auto bg-[#19246a]/95 backdrop-blur-md border border-white/10 flex justify-around items-center py-3 px-2 rounded-[28px] shadow-2xl">
          {mobileNavItems.map((item) => {
            ...
            return (
              <Link key={item.label} href={item.href} className="flex flex-col items-center gap-1 flex-1 transition-transform active:scale-90">
                <div className={`p-1 transition-colors ${isActive ? 'text-emerald-400' : 'text-slate-400'}`}>
                  <item.icon size={20} strokeWidth={isActive ? 2.5 : 2} />
                </div>
                <span className={`text-[8px] font-black uppercase tracking-widest ${isActive ? "text-white" : "text-slate-400/70"}`}>
                  {item.label}
                </span>
              </Link>
            );
          ...
          >
            <div className={`p-1 transition-colors ${showMore ? 'text-emerald-400' : 'text-slate-400'}`}>
              <MoreHorizontal size={20} strokeWidth={showMore ? 2.5 : 2} />
            </div>
            <span className={`text-[8px] font-black uppercase tracking-widest ${showMore ? "text-white" : "text-slate-400/70"}`}>
              More
            </span>
          </button>
        </nav>
